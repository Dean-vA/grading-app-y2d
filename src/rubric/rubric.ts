...
const ITEMS: RubricItem[] = [
  ...
  {
    key: 'ILO9_5_A',
    label: 'Containerised inference API deployed locally (Docker) & on-premise (Portainer): frontend + backend + database + inference',
    maxPoints: 5,
    input: 'number',
    ilo: '9.5',
    tab: 'Deployment',
  ...
  },
  {
    ...
    tab: 'Deployment',
    section: 'B: GitHub Repository & DevOps (5 Points)',
    criteriaNote: 'Check for successful GitHub Actions runs (lint and test workflows passing).',
  ...
  {
    key: 'ILO10_A_latency',
    label: 'Operational metrics: request latency over time, error rates, or tracing to identify bottlenecks',
    maxPoints: 2,
    input: 'number',
    ilo: '10',
    tab: 'Frontend',
  ...
  },
  {
    ...
    criteriaLines: [
      {
        label: '3 points:',
        description: 'Versioned train and test (and validation) sets registered as separate data assets',
        tone: 'good',
      },
      {
        label: 'Max 1 point:',
        description: 'No separate train and test assets',
        tone: 'warn',
      },
    ],
  ...
  {
    key: 'ILO8_A_training',
    label: 'Training job successfully run (locally and in the cloud)',
    maxPoints: 3,
    input: 'radio',
    ilo: '8',
    tab: 'AzureML',
    ...
    requirement:
      'Official 8.9 A: train models both locally and in the cloud using industry-standard libraries to track key training and evaluation metrics.\nAlso check: environments created and registered in Azure ML containing the dependencies for training/evaluation/inference (ILO 8.9A).\nIf there are no on-premise (self-hosted MLflow) training jobs, award a MAXIMUM of 2 points.',
    options: [
      {
        points: 3,
        label: '3 points:',
        description:
          'Evidence of training both locally and in the cloud with comparable metrics; logs make sense, metrics are good, and MLFlow tracking is present (incl. on-prem self-hosted MLflow jobs)',
        tone: 'good',
      },
      {
        points: 2,
        label: 'Max 2 points:',
        description:
          'Cloud training runs successfully but metrics are weak, MLFlow tracking is missing, or there are no on-prem MLflow jobs',
        tone: 'warn',
      },
      {
        points: 1,
  ...
  {
    key: 'ILO8_A_model',
    label: 'Model registered in model store with version control',
    maxPoints: 1,
    input: 'checkbox',
    ilo: '8',
    tab: 'AzureML',
    section: 'Training Job (4 Points)',
    criteriaNote: 'Model is registered in the model store and version-controlled (ILO 8.9A)',
    commentPlaceholder: 'Comments on model registration and versioning...',
  },
  {
    key: 'ILO8_B_pipeline',
    label:
      'Training & evaluation pipeline with multiple components, incl. automated hyperparameter tuning and multiple model architectures (custom + open-source)',
    maxPoints: 3,
    input: 'number',
    ilo: '8',
    tab: 'AzureML',
    section: 'Advanced Features (6 Points)',
    commentPlaceholder: 'Comments on training/evaluation pipelines and hyperparameter tuning...',
  },
  {
    key: 'ILO8_B_mlflow',
    label:
      'Experiment tracking: automated metric logging & visualisation (learning curves, example outputs)',
    maxPoints: 1,
    input: 'number',
    ilo: '8',
    tab: 'AzureML',
    section: 'Advanced Features (6 Points)',
    commentPlaceholder: 'Comments on experiment tracking (MLflow)...',
  },
  {
    key: 'ILO8_B_scheduled',
    label: 'Training pipeline scheduled or triggered based on appropriate criteria',
    maxPoints: 1,
    input: 'number',
    ilo: '8',
    tab: 'AzureML',
    section: 'Advanced Features (6 Points)',
    commentPlaceholder: 'Comments on pipeline scheduling/triggering...',
  },
  {
    key: 'ILO8_B_conditional',
    label: 'Conditional model registration (model registered only if it meets evaluation criteria)',
    maxPoints: 1,
    input: 'number',
    ilo: '8',
    tab: 'AzureML',
    section: 'Advanced Features (6 Points)',
    commentPlaceholder: 'Comments on conditional model registration...',
  },
  {
    key: 'ILO4_C_GPU',
  ...
  },
  {
    ...
    requirement:
      'The cost analysis should include:\n• Itemised costs of the Azure resources used (compute, storage, endpoints/Container Apps, etc.)\n• Training vs inference costs, including the GPU vs CPU choice and its justification\n• Estimated monthly running cost at expected usage (and how it scales)\n• Cost-saving measures considered (e.g. low-priority/spot compute, autoscaling or scale-to-zero, right-sizing)\n\nNote: GPU usage assessment (1 point) should be checked with Azure ML workspace artifacts, not here.\nCost analysis document assessment only.',
  ...
  /* ---------------------------- Code Quality & Documentation (ILO 9.3) ---------------------------- */
  {
    ...
    requirement:
      'Check codebase for: Best practice, Modularity, Logging, Doc strings, PEP8 compliance, Pre-commits set up, CLI for inference/training (ILO 9.3A)\n\nHow to check:\n• Modularity and docstrings: code spot checks on GitHub\n• PEP8: linting reports (black, flake8, or ruff)\n• CLI: must be able to run inference AND training (a small sample set is fine)\n• Logging: check output when the CLI runs (can also be checked in Azure jobs)',
    ...
    options: [
      {
        points: 5,
        label: '5 points:',
        description: 'Coverage report with 80% coverage or higher',
        tone: 'good',
      },
  ...
  {
    key: 'ILO9_4_B_onprem',
    label: 'On-premise data pipeline (Airflow)',
    maxPoints: 4,
    input: 'radio',
    ilo: '9.4',
    tab: 'Pipeline',
    ...
    requirement:
      'On-premise pipeline requirements:\n• Must be Airflow (check DAG runs — all dark green) and must run successfully\n• Ingests raw data and outputs versioned train and test sets\n• Students should explain data flow, processing, and final data format',
    options: [
      {
        points: 4,
        label: '4 points:',
        description:
          'Working Airflow pipeline that runs successfully, ingests raw data, and outputs versioned train and test sets',
        tone: 'good',
      },
      {
        points: 2,
        label: 'Max 2 points:',
        description:
          'Partial pipeline — runs but missing versioned train/test outputs or some processing steps',
        tone: 'warn',
      },
      { points: 0, label: '0 points:', description: 'No working on-premise pipeline', tone: 'neutral' },
    ],
    commentPlaceholder: 'Comments on the on-premise (Airflow) data pipeline...',
  },
  {
    key: 'ILO9_4_B_cloud',
    label: 'Cloud data pipeline (Azure ML Pipelines or Airflow)',
    maxPoints: 4,
    input: 'radio',
    ilo: '9.4',
    tab: 'Pipeline',
    section: 'Data Pipeline (10 Points)',
    requirement:
      'Cloud pipeline requirements:\n• Azure ML Pipelines or Airflow — must run successfully\n• Must be SEPARATE from the training pipeline\n• Ingests raw data and outputs versioned train and test sets',
    options: [
      {
        points: 4,
        label: '4 points:',
        description:
          'Working cloud pipeline (Azure ML or Airflow), separate from the training pipeline, that ingests raw data and outputs versioned train and test sets',
        tone: 'good',
      },
      {
        points: 2,
        label: 'Max 2 points:',
        description:
          'Partial pipeline — runs but is combined with the training pipeline, or missing versioned train/test outputs',
        tone: 'warn',
      },
      { points: 0, label: '0 points:', description: 'No working cloud data pipeline', tone: 'neutral' },
    ],
    commentPlaceholder: 'Comments on the cloud data pipeline...',
  },
  {
    key: 'ILO9_4_B_scheduled',
    label: 'Pipeline scheduled or triggered based on reasonable criteria',
    maxPoints: 2,
    input: 'number',
    ilo: '9.4',
    tab: 'Pipeline',
  ...
  },
  {
    ...
    sections: [
      ...
      { title: 'Training Job (4 Points)', items: ['ILO8_A_training', 'ILO8_A_model'] },
      {
        ...
        items: ['ILO8_B_pipeline', 'ILO8_B_mlflow', 'ILO8_B_scheduled', 'ILO8_B_conditional'],
      },
      {
    ...
    sections: [
      {
        ...
        alert: {
          tone: 'info',
          text: 'Both an on-premise (Airflow) and a cloud data pipeline are required — assess each separately. Both must ingest raw data and output versioned train and test sets.',
        },
        items: ['ILO9_4_B_onprem', 'ILO9_4_B_cloud', 'ILO9_4_B_scheduled'],
      },
    ],
  },
  {
    ...
    commentOnly: [
      {
        ...
        alert: {
          tone: 'warn',
          text: 'ILO 1: Needs to be checked after demo day. Note down professional behaviour observations here.',
        },
        placeholder: 'Notes about ILO 1 assessment and professional behaviour...',
      },
      {
        key: 'ILO2',
        alert: {
